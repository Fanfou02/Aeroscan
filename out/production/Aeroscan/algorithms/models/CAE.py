...
class CAE:
	...
	def save(self, filename):
		assert self.autoencoder is not None, "CAE not initiate, please call createModel() first"
		# Saving the delta error:
		with open('delta.pkl', 'wb') as f:  # Python 3: open(..., 'wb')
		    pickle.dump(self.deltaError, f)
	...
	def load(self, filename):
		...
		# Getting back the delta error:
		with open('delta.pkl', 'rb') as f:  # Python 3: open(..., 'rb')
		    self.deltaError = pickle.load(f)
